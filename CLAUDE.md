...
>
> **`vendor/ppt/`는 upstream HEAD(5880e31)의 사본이다.** 이미
> `web/src/areas/slides/`로 옮겨 간 94개 파일만 빠져 있다 — 되살리면 같은 코드가
> 두 벌이 되고, 뒤처지는 쪽이 생긴다. upstream을 다시 당길 때도 같은 규칙이다:
> **옮겨 간 파일의 변경은 vendor가 아니라 새 집에 반영한다.**
...
> **자격마다 여는 영역이 다르고, 그 표는 두 곳뿐이다** — `passwordGrant`
> (비밀번호)와 `verifyAdminJwt`(구글 계정). 셋째 곳이 생기면 한쪽이 뒤처지고,
> 뒤처진 쪽이 열려 있는 쪽이 된다.

처음에는 **비밀번호는 언제나 출석뿐**이었다. 주일 화면에 나가는 일과 AI 무료
한도를 쓰는 일은 회수할 수 있고 로그인 기록이 남는 자격 뒤에 있어야 하기
때문이었다 — 공용 비밀번호는 바꾸는 순간 그걸 쓰던 모두가 막히지만, 계정은 한
사람만 끊을 수 있다. **그 규칙은 운영 때문에 깨졌다**: 미디어팀·찬양팀은 매 학기
사람이 바뀌는데 구글 계정을 넘기는 일이 매번 배포할 수 있는 한 사람을 거쳐야
했다. 대가는 그대로 남아 있으니 적어 둔다 — **새면 회수하는 길은 값을 바꾸는
것뿐이고, 그러면 그 비밀번호를 쓰던 모두가 함께 막힌다. 새 학기마다 바꿔라.**

- `Area = 'attend' | 'slides' | 'praise'`, `Role.areas: Area[]` (`auth.ts`).
  셋이 다 살아 있다: **출석**(관리자 패널) · **슬라이드**(미디어팀의 마법사) ·
  **찬양**(찬양팀 인도자의 콘티 화면).
- **비밀번호 여섯 — 출석 셋, 영역 셋** (`passwordGrant`). 환경 변수로 덮어쓴다.

  | 비밀번호 | 역할 | 영역 | 부(部) | 팀 |
  |---|---|---|---|---|
  | `kccpadmin` | `super_admin` | 출석 | 대학·청년부 고정 | — |
  | `kccpwelcome` | `welcoming` | 출석 | 대학·청년부 고정 | — |
  | `kccpadults` | `super_admin` | 출석 | 장년부 고정 | — |
  | `kccp1980` | `super_admin` | 출석·슬라이드·찬양 | **고른다** | **고른다** |
  | `kccpmedia` | `media` | 슬라이드·찬양 | **고른다** | **고른다** |
  | `kccppraise` | `praise_leader` | 찬양 | 팀이 정한다 | **고른다** |

  - **출석 셋은 출석에서 자라지 않는다.** 명단은 사람의 정보라, 슬라이드를 쓰라고
    준 자격으로 열리면 기능이 아니라 유출이다. 거꾸로 `kccpmedia`·`kccppraise`는
    `areas`에 `'attend'`가 없어 `/api/admin/*` 전부에서 떨어진다 —
    막는 것은 화면이 아니라 `resolveAdmin`이다.
  - **영역 비밀번호는 부(部)를 고른다** (`PasswordGrant.crossPartition`). 보통
    비밀번호는 그 자체가 부를 뜻하지만(장년부 비밀번호 → 장년부), 영역 비밀번호는
    부가 아니라 **일**을 뜻한다 — 미디어팀 하나가 두 부의 슬라이드를 만드는 주가
    실제로 있다. 고르는 길은 구글 계정과 같다(`X-Partition` → `canChoosePartition`),
    그리고 **출석 셋은 그 헤더를 읽지 않는다** — 읽으면 장년부 비밀번호로
    대학·청년부가 열린다. 테스트가 그 문장을 붙잡는다.
  - **영역 비밀번호는 팀을 고른다** (`PasswordGrant.anyTeam` → `canPickTeam`).
    공용 비밀번호는 사람을 가리키지 못해 **팀도 가리키지 못하므로**, 「자기 팀」으로
    좁히면 그 화면은 아무것도 못 한다. 그래서 소유자와 같은 모양으로 **화면이
    묻는다.** 대가는 분명하다: 이 값을 아는 사람은 남의 팀 콘티도 연다. 팀이 자격에
    적힌 인도자(`team_leaders`)에게는 이 값이 붙지 않는다 — 아는 것을 묻지 않는다.
  - **영역 비밀번호는 기기에 걸린 출석 역할을 물려받지 않는다.** `member_roles`는
    출석 쪽 역할표라, 최고관리자의 폰에서 `kccpmedia`를 치면 **역할은 넓고 영역은
    좁은** 자격이 생긴다 — 아무도 의도하지 않은 조합이고, 화면에서는 "왜 탭이
    없지"로만 보인다. `verifyAdmin`이 `areas`에 `'attend'`가 있을 때만 그 조회를 한다.
...
- `team_leaders` (`20260829`) — **찬양팀 인도자.** 이메일 하나가 팀 하나를 가리키고,
  그 줄이 곧 `'praise'` 영역이다 (`praiseTeamOf`). `kccppraise`와 다른 점은 **팀을
  안다는 것**이다 — 그래서 그 화면은 팀을 묻지 않고, 그것이 이 표가 있는 이유다. 미디어팀을 환경 변수에 둔 것과
  **반대로 표에 두는 이유**: 미디어 계정은 사람이 아니라 역할 계정이지만 인도자는
  사람이고, 사람은 학기마다 바뀐다 — 환경 변수는 바꿀 때마다 배포가 필요하고 배포할
  수 있는 사람은 하나다. `config.dongsan_leaders`와 같은 판단이다.
  - **한 인도자는 한 팀** (`team_leaders_one_team_per_email`, 부분 유니크 인덱스).
    둘을 이끌 수 있게 두면 「올린 인도자에 맞춰서」가 답을 잃고, 그 판단이 결국
    주일 아침으로 밀린다. 실제로 겹치는 사람이 생기면 제약을 풀고 **화면에 고르는
    자리를 만들어야** 한다 — 그 결정이 조용히 내려지지 않도록 여기서 막는다.
  - 자격은 **신원을 대체하지 않고 영역만 넓힌다** (소유자와 같은 모양). members 행이
    있는 인도자는 그 사람으로 남아야 로그인 기록에 이름이 남는다.
  - 부(部)는 **팀이 정한다.** 인도자가 어느 부 교인이냐가 아니라 어느 팀을 이끄느냐가
    그 콘티가 어느 예배의 것인지를 정한다 (헵시바=장년, 주랑=대학·청년).
  - `email` 은 `ilike` 로 찾고 **다시 정확히 견준다.** `_` 는 이메일에 쓸 수 있는
    글자인데 ilike 패턴에서는 아무 글자 하나를 뜻한다 — 견주지 않으면 남의 팀 콘티를
    여는 길이 된다.
...
- **`ai-proxy`는 접두사 둘을 받는다** (`AREA_PREFIXES`): `/api/slides/ai/…`와
  `/api/praise/ai/…`. 규칙을 두 번 적지 않고 접두사만 벗겨 낸다 — 두 벌이 되면 한쪽이
  뒤처지고, 뒤처진 쪽이 열려 있는 쪽이 된다. 브라우저 쪽 짝은 `lib/ai/proxy.ts`의
  `setAiArea()`이고, 화면이 열릴 때 한 번 정한다 (`lib/api.ts`의 `adminPartition`과
  같은 모양). 인도자에게 이 문을 여는 것은 판단이다: 가사를 읽는 일이 곧 「자동으로
  만든다」이므로, 그것 없이는 찬양 영역이 업로드 창구에 지나지 않는다.
...
가진 앱이라, 폴더가 출석 모양이면 그 모양대로 자란다.
    ...
    areas/praise/   PraiseLeaderShell · lib/conti · lib/autoBuild
...
- **순서가 곧 예배 순서다**: front → 찬양 → 기도 → 말씀 → 설교 → 기도 →
  **설교 후 찬양 → 기도** → 광고 → back → 추가 자료. 바꾸는 것은 리팩터링이 아니라
  예배를 바꾸는 일이다.
  - **설교 후 찬양**은 `song.postSermon` 이 정하고, 콘티에서는 **공동체 고백송 바로
    다음 곡**이 그것이다 (`splitLyricsAndConfessionSongs`). 그 곡과 뒤따르는 기도는
    예배 순서의 **한 칸**이라 없는 주에는 둘을 함께 건너뛴다 — 따로 두면 기도
    슬라이드가 연달아 두 장 나온다.
  - **공동체 고백송은 back 덱 안에서 제자리에 고쳐 쓴다** (`applyConfessionSong`).
    그 곡의 가사 슬라이드는 고정 back 덱에 이미 있으므로 뒤에 덧붙이는 것이 아니다 —
    쪼갰다 붙이면 3 MB의 마스터·미디어가 통째로 복제된다. 제목은 공유 설정
    (`confessionSong`, 서버의 `ai-proxy/catalog.ts` 와 짝), 가사는 곡 라이브러리
    (`web/public/slides/library.json`). 가사를 못 찾으면 back 덱을 **받은 그대로**
    둔다 — 빈 곡으로 고쳐 쓰면 주일 아침에 빈 슬라이드가 나온다. 콘티의 그 곡은
    찬양 목록에서 빠진다 (`contiSongs.ts`; 두 화면이 그 한 규칙을 나눠 쓴다).
...
`areas/praise/`: `PraiseLeaderShell`(한 화면) + `lib/conti.ts`(서버와 오가는 길) +
`lib/autoBuild.ts`(콘티 → 완성된 덱). 서버는 `attendance-api/praise.ts` + `/api/praise/`.

이 합치기의 원래 목적이 **미디어팀이 카톡으로 받은 PDF를 다시 올리지 않는 것**이었다면,
여기는 그 다음 칸이다: 인도자가 콘티를 올리면 슬라이드까지 **그 자리에서** 나온다.

- **이 화면의 값은 물음이 거의 없다는 데 있다.** 팀은 자격이 알고(`team_leaders`), 부는
  팀이 정하고, 예배는 팀이 서는 예배이며, 날짜는 **서버가 정한 다음 주일**이고
  (`upcomingSunday` — 화면의 시계는 폰의 시계다), 곡·본문·설교 제목은 콘티 표지에 적혀
  있다. 남는 물음은 하나뿐이다: **이끄는 예배가 둘인 팀**(헵시바의 1·2부)에게 어느
  예배인가. `defaultService`가 답할 수 있으면 그것도 묻지 않는다 — 임의로 고르면 1부
  콘티가 2부에 앉고, **그 잘못은 올린 사람 눈에 보이지 않는다.**
- 미디어팀의 마법사가 여섯 단계인 이유는 **고칠 수 있어야** 하기 때문이다(설교 PPT ·
  광고 · 번역본). 인도자에게는 그 물음이 하나도 해당하지 않으므로 단계가 없다.
- **범위가 부(部)가 아니라 팀이다.** `/api/praise/`의 모든 경로가 `role.team` 하나로
  좁혀진다 (`canTouchSetlist`) — 팀 id는 자격에서 오고 요청 본문에서는 오지 않는다.
  보낼 수 있으면 그것이 곧 남의 팀 콘티를 덮어쓰는 길이다. 최고관리자도 예외가 아니다:
  그 사람이 콘티를 다루는 자리는 슬라이드 영역이고, 거기서는 부가 범위다. **소유자만**
  예외인데, 잘못 앉은 콘티를 옮길 자격이 하나는 있어야 하기 때문이다.
- **올리는 것이 먼저, 만드는 것이 나중이다.** 조립이 실패해도 콘티는 이미 저장돼 있어야
  미디어팀이 손으로 이어받는다 — 자동이 실패했을 때 예전 방식으로 돌아가는 길이 열려
  있어야 한다.
- **인식이 실패해도 덱은 나온다** (`autoBuild.ts`). 가사를 못 읽은 곡은 제목만 든 빈
  슬라이드가 되고 화면이 그 곡 이름을 적어 준다. 여기서 던지면 인도자는 토요일 밤에
  아무것도 없이 남고, 그 주의 슬라이드는 결국 미디어팀이 처음부터 만든다 — 없애려던
  그 일이다. 같은 이유로 **가사가 비어 있는 곡은 전부 「확인해 주세요」로 올린다**:
  조용히 빈 슬라이드가 나가는 것이 이 화면에서 가장 나쁜 실패다.
- 완성된 덱은 **두 곳으로** 간다. 인도자가 내려받고(`MMDD.pptx`, ppt의 규칙 그대로 —
  미디어팀이 그 이름으로 한 주를 찾는다), 같은 파일이 그 주의 콘티 옆 Storage에 앉는다.
  저장이 실패해도 내려받기는 남는다 — 카톡으로 보내는 길이 아직 살아 있어야 한다.
- `setlists.uploaded_by_email` — 기록이 아니라 **주일 아침에 물어볼 곳**이다. 표지의 곡
  순서가 이상할 때 연락할 데가 팀 이름이면 아무 데도 아니다.
...
찬양 영역도 같은 규칙을 탄다 (`assets/praise-[hash].js`, `globIgnores`의 `praise-*`).
그 화면은 슬라이드의 조립·인식을 **그대로 나눠 쓰므로** 새로 지는 무게는 화면 하나뿐이다
(9.7 KiB). 지금 선캐시는 1105 KiB — 두 영역이 다 lazy인 한 이 숫자는 거의 움직이지 않는다.
...
## 통계 탭의 새가족 블록 (`analytics.ts` 아래쪽 · `AdminAnalytics.tsx` `NewFamilySection`)
- 위쪽 통계가 전체를 세는 자리라면 그 아래부터는 **새가족만** 센다: 숫자 타일(전체 · 이번 학기
  등록 · 최근 4주 출석 · 새가족 교육 이수), 그래프 둘(월별 등록 막대 · 주일별 새가족 출석 선),
  월별 표(등록 · 출석 인원 · 비중). 입력은 위와 **같은** members + log이라 부서/동산 필터가
  그대로 걸리고, 숨긴 멤버도 그대로 빠진다.
- **여기서 새가족은 `members.is_new_member` 하나다** — 새가족 탭의 `visibleNewFamily`가 아니다.
  그쪽은 "지금 새가족팀이 챙길 사람"이라 숨김 표기가 걸린 사람과 2026년 이전 등록을 걷어내는데,
  **추이는 지나간 사실**이라 그 사람이 그 달에서 사라지면 이번 주에 지난달 숫자가 바뀐다.
- **등록일이 없는 새가족은 어느 달에도 놓을 수 없어** 그래프·월별 표에서 빠진다. 그래서 그
  인원수를 블록 머리에 적어 둔다 (`nfUndated`) — 안 적으면 타일의 총계와 그래프의 합이 어긋나
  보인다. 같은 이유로 등록이 없던 달도 0으로 채운다 (`monthsBetween`): 건너뛰면 x축이 붙어
  "그 달에도 왔다"로 읽힌다.
- 출석 줄이 새가족의 것인지는 **`memberId`가 열쇠**고(동명이인이 갈린다), 그것이 없는 옛 줄·
  손님 줄만 이름으로 되짚는다. 그날 전체 출석도 같이 세어 두는 이유는 **비중이 새가족 숫자
  만으로는 나오지 않기** 때문 — 점을 누르면 `n / 전체 (%)`가 나온다.
- **출석 추이는 새가족 교육 단계로 갈라 볼 수 있다** — 그래프 위의 칩(전체 · 수강 완료 ·
  1주차만 · 2주차만 · 미수강)이 `newFamilyTrend(members, log, edu)`에 그대로 넘어가고, 갈래와
  문구는 새가족 교육 탭의 `matchesEduFilter` · `eduFilter.*`를 **그대로 쓴다** (같은 네 갈래를
  두 화면이 다르게 부르면 "1주차만"이 어디서는 다른 뜻인가 싶어진다). 한 단계를 고르면 그 뒤에
  **새가족 전체가 흐린 점선으로 깔린다** — 미수강 3명은 새가족이 4명일 때와 40명일 때가 다른
  사실이라 고른 선만으로는 읽을 수 없다. 선이 둘일 때만 범례가 켜진다.
  - **갈래는 언제나 *지금* 상태다**: `new_member_edu_week1/2`는 날짜 없는 참/거짓이라, 지난달에
    아직 미수강이던 사람도 이번 주에 이수를 찍으면 그 달까지 '수강 완료'로 그려진다. 교육을 마친
    사람들이 요즘 어떻게 오고 있나를 보는 선이지, 그때 그 사람의 단계가 아니다.
  - 갈래 판정은 `newFamilyMatcher`를 거친 뒤에 걸리므로 **교육 칸이 비어 있는 일반 멤버가
    '미수강'으로 딸려 들어오지 않는다** (단계는 새가족 안에서만 뜻이 있다).
- 장년부에는 **새가족 교육 타일도 단계 칩도 없고**(그 부에 교육이 없다) "이번 학기"가
  "이번 반기"다 (`nfThisTerm_adult`, `usePartitionT`). 학기 경계는 통계가 정하지 않고
  `newFamily.ts` `semesterBounds`에서 받아 온다 — 부마다 한 해를 나누는 방식이 다르기 때문.

## 새가족 목록에서 사람을 내리는 열쇠 (`newFamily.ts` `visibleNewFamily`)
- **새가족 표시(`is_new_member`)를 해제하는 것 하나뿐이다.** 학기도, 새가족 교육 이수도 목록에서
  사람을 내리지 않는다 — 새가족 탭과 새가족 교육 탭이 같은 이 함수를 읽는다.
- 예전에는 "지난 학기에 등록한 사람은 교육 두 주를 다 마치면 목록에서 빠진다"였는데, 그러면
  **이수한 사람을 확인할 방법이 교육 탭에 없었다**: 마치는 순간 목록 밖으로 나가므로 '수강 완료'로
  걸러도 아무도 안 나온다. 이수 여부를 적는 탭이 이수한 사람을 못 보여주는 것이라, 그 규칙을
  없앴다. 넘어온 학기 머리줄의 '교육 미완료' 배지도 같이 없앴다 (더는 그 묶음의 사실이 아니다).
- 남은 필터는 셋이다: 새가족 표시가 있을 것 · 숨김 표기(귀국·이주·졸업 …)가 없을 것 ·
  등록일이 2026-01-01 이후일 것(옛 시트에서 옮겨온 기록은 올리지 않는다). 그래서 함수는
  오늘 날짜도 학기 일정도 받지 않는다 — `visibleNewFamily(members)` 하나다.
- `isActiveNewFamily`는 남아 있지만 이제 **출석부 엑셀의 새가족 표시**(`exports.ts`, 등록 전
  주일 칸에 붙는 메모) 전용이다. 그쪽은 "아직 새내기인가"를 묻는 자리라 이수가 답을 바꾼다.
